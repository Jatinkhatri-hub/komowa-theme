...
document.addEventListener("facet:update", async (event) => {
  ...
  try {
    ...
    newShopifySection.querySelectorAll(".product-title").forEach((titleElement) => {
      let productLink = titleElement.getAttribute("href");
      let modifiedLink = productLink.split("?")[0];
      titleElement.setAttribute("href", modifiedLink);
      // console.log(modifiedLink);
    });


    newShopifySection.querySelectorAll(".product-card__media").forEach((cardElement) => {
      let productLink = cardElement.getAttribute("href");
      let modifiedLink = productLink.split("?")[0];
      cardElement.setAttribute("href", modifiedLink);
      // console.log(modifiedLink);
    });
...
onScroll_fn2 = function() {
  ...
  $('.icon-with-text.komowa-icon-with-text').toggleClass('header-sticky', window.scrollY > __privateGet(this, _offset));
};
